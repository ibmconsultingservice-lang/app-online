...
// ── Client-side CSV parser (handles quoted fields with commas) ──
...
export default function DashboardAI() {
  ...
  // ── Parse uploaded file client-side before sending to API ──────
  const handleFile = useCallback(async (f) => {
    ...
    try {
      ...
      if (name.endsWith('.csv') || name.endsWith('.txt') || f.type === 'text/csv') {
        ...
        setParsedRows(rows.slice(0, 50))   // send up to 50 rows for richer schema
        setDataPreview(
          rows.length
            ? Object.keys(rows[0]).join(', ') + '\n' + rows.slice(0, 5).map(r => Object.values(r).join(', ')).join('\n')
            : 'Fichier CSV vide ou non lisible.'
        )
        ...
        setParsedRows(arr.slice(0, 50))
        setDataPreview(JSON.stringify(arr.slice(0, 3), null, 2))

      } else if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
        // Excel: we can't parse binary client-side without SheetJS.
        // Send filename + size; API will generate a generic schema + demo data.
        setDataPreview(`Fichier Excel: ${f.name} (${(f.size / 1024).toFixed(1)} Ko)\nLes colonnes seront analysées et un dataset de démonstration sera généré.`)

      } else {
        const text = await f.text()
        setDataPreview(text.slice(0, 600))
      }
    } catch (err) {
  ...
  const markStep = (stepId) => {
    ...
    setCompletedSteps(prev => [...new Set([...prev, stepId - 1])])
  }
  ...
  const handleGenerate = async () => {
    ...
    const stepTimer1 = setTimeout(() => markStep(2), 10000)
    const stepTimer2 = setTimeout(() => markStep(3), 28000)
    const stepTimer3 = setTimeout(() => markStep(4), 50000)
    ...
    try {
      ...
      const res = await fetch('/api/generer-dashboardai', {
        ...
        body: JSON.stringify({
          ...
          parsedRows:    parsedRows.slice(0, 50),
          fileName:      file?.name || null,
          dashboardType: selectedType?.label || dashboardType,
          colorTheme,
          language,
        }),
      })
  ...
  const handleDownload = () => {
    ...
    a.href = url; a.download = `dashboard-${dashboardType}-${Date.now()}.html`
    a.click(); URL.revokeObjectURL(url)
  }

  const removeFile = () => {
    setFile(null); setDataPreview(''); setParsedRows([])
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">
              Fichier de données (optionnel — CSV, JSON, Excel)
            </label>
            ...
            {!file ? (
              ...
              >
                <div className="border-2 border-dashed border-white/10 group-hover:border-indigo-500/40 rounded-2xl p-8 text-center transition-all">
                  ...
                  <p className="text-xs text-slate-600 mt-1">CSV · JSON · Excel (.xlsx)</p>
                  <p className="text-[10px] text-slate-700 mt-2">Claude analyse jusqu'à 50 lignes pour construire le dashboard</p>
                </div>
                <input ref={fileRef} type="file" hidden accept=".csv,.json,.txt,.xlsx,.xls" onChange={handleFileInput}/>
              </label>
            ) : (
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 space-y-3">
                ...
                {dataPreview && (
                  <div className="bg-black/30 rounded-xl p-3 font-mono text-[10px] text-slate-400 max-h-24 overflow-y-auto whitespace-pre-wrap">
                    {dataPreview.slice(0, 500)}
                  </div>
                )}
              </div>
            )}
              ...
              {WORKFLOW_STEPS.map((step, i) => {
                ...
                return (
                  <div key={step.id} className={`flex items-start gap-3 p-3 rounded-xl transition-all ${
                    ...
                    isDone   ? 'bg-emerald-500/10 border border-emerald-500/20' : 'opacity-40'
                  }`}>
