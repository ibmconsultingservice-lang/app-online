...
const services = [
  ...
  { id: 'cv', cat: 'Documents', name: 'CV Builder', desc: 'Créez des Curriculum Vitae haute performance en quelques clics.', icon: FileText, path: '/cv', img: 'https://images.unsplash.com/photo-1586281380349-632531db7ed4?q=80&w=800&auto=format&fit=crop' },
    ...
    <main className="min-h-screen bg-[#f8fafc] text-slate-900 selection:bg-indigo-100 overflow-x-hidden font-sans">
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex gap-8 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 items-center">
            <div className="relative group/menu">
              <button onMouseEnter={() => setIsMenuOpen(true)} className="flex items-center gap-1 hover:text-indigo-600 transition-colors py-2">
                Fonctionnalités <ChevronDown size={12} className={`transition-transform ${isMenuOpen ? 'rotate-180' : ''}`} />
              </button>
              <div onMouseLeave={() => setIsMenuOpen(false)} className={`absolute top-full left-1/2 -translate-x-1/2 w-[600px] bg-white border border-slate-200 shadow-2xl rounded-2xl p-6 grid grid-cols-3 gap-8 transition-all duration-300 ${isMenuOpen ? 'opacity-100 translate-y-2 visible' : 'opacity-0 translate-y-4 invisible'}`}>
                {categories.slice(0, 3).map(cat => (
                  <div key={cat}>
                    <h4 className="text-indigo-600 mb-4 border-b border-slate-100 pb-2">{cat}</h4>
                    <div className="flex flex-col gap-3">
                      {services.filter(s => s.cat === cat).map(item => (
                        <button key={item.id} onClick={() => { setIsMenuOpen(false); router.push(item.path); }} className="normal-case tracking-normal text-slate-600 hover:text-indigo-600 text-sm font-medium flex items-center gap-2 text-left">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <Link href="/dashboard" className="h-10 px-5 bg-indigo-600 text-white hover:bg-indigo-700 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all shadow-md flex items-center gap-2">
                Dashboard
              </Link>
              <button onClick={handleLogout} className="h-10 w-10 flex items-center justify-center rounded-lg border border-slate-200 hover:bg-red-50 hover:border-red-200 hover:text-red-500 transition-all" title="Se déconnecter">
                <LogOut size={15} />
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              <Link href="/login" className="h-10 px-5 border border-slate-200 text-slate-700 hover:border-indigo-300 hover:text-indigo-600 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all hidden md:flex items-center">
                Connexion
              </Link>
              <Link href="/register" className="h-10 px-6 bg-slate-900 text-white hover:bg-indigo-600 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all shadow-md flex items-center">
                Commencer →
              </Link>
            </div>
          )}
        ...
        <div className="max-w-4xl">
          ...
          {!user && (
            <div className="flex flex-wrap gap-4">
              <Link href="/register" className="inline-flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-2xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/25">
                Créer un compte gratuit →
              </Link>
              <Link href="/login" className="inline-flex items-center gap-2 px-8 py-4 bg-white border border-slate-200 text-slate-700 rounded-2xl text-sm font-bold hover:border-indigo-300 hover:text-indigo-600 transition-all">
                Se connecter
              </Link>
            </div>
          )}
          {user && (
            <Link href="/dashboard" className="inline-flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-2xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/25">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 py-24 border-t border-slate-200/60">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="flex flex-col items-center md:items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600"><Shield size={24} /></div>
          ...
          <div className="flex flex-col items-center md:items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600"><Users size={24} /></div>
          ...
          <div className="flex flex-col items-center md:items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600"><Star size={24} /></div>
      ...
      <section className="relative z-10 bg-slate-900 text-white py-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[8px] font-black uppercase tracking-[0.3em] mb-6">Lab Innovation</div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight leading-tight mb-6">
              Une équipe dédiée à votre <br/>
              <span className="text-indigo-400 italic">succès digital.</span>
            </h2>
            <p className="text-slate-400 text-lg font-medium leading-relaxed mb-8">Notre équipe d'ingénieurs et d'experts travaille 24/7 pour intégrer chaque mois les derniers outils du marché.</p>
