...
export default function FactureModerne() {
  ...
  return (
    <div className="min-h-screen bg-[#e0e0e0] py-10 px-4 font-sans text-[#333]">
      ...
      {/* Barre d'outils avec Sélecteur de Couleur */}
      <div className="max-w-[850px] mx-auto mb-6 flex flex-wrap gap-4 justify-between items-center no-print">
         <div className="flex gap-4">
            <button onClick={() => window.print()} className="bg-white text-slate-800 px-6 py-2 rounded-lg font-bold shadow-md hover:bg-gray-50 transition-all border border-gray-200">
              Exporter PDF 🖨️
            </button>
            <button onClick={addItem} className="bg-white text-emerald-600 px-6 py-2 rounded-lg font-bold shadow-md hover:bg-gray-50 transition-all border border-gray-200">
              + Ligne
            </button>
         </div>
         ...
         <div className="flex items-center gap-3 bg-white p-2 rounded-xl shadow-sm border border-gray-200">
            ...
            {colors.map((c) => (
              <button
                ...
                className={`w-6 h-6 rounded-full border-2 transition-transform hover:scale-110 ${primaryColor === c.hex ? 'border-white ring-2 ring-gray-400' : 'border-transparent'}`}
      ...
      <div className="bg-white max-w-[850px] mx-auto shadow-[0_10px_30px_rgba(0,0,0,0.1)] rounded-lg overflow-hidden border border-gray-200 invoice-card">
        ...
        {/* En-tête Dynamique */}
        <div className="p-10 flex justify-between items-center transition-colors duration-500" style={{ backgroundColor: primaryColor, color: '#fff' }}>
          <div onClick={() => fileInputRef.current.click()} className="bg-white p-2 rounded cursor-pointer relative group no-print-logo-border">
          ...
          <div className="text-right">
            ...
            <p className="opacity-80 outline-none" contentEditable suppressContentEditableWarning>N° INV-2026-001</p>
          </div>
        </div>
        ...
        <div className="flex justify-between p-10">
          <div className="space-y-1">
            <h3 className="border-b-2 border-[#f4f7f8] pb-1 mb-2 font-bold uppercase text-sm" style={{ color: primaryColor }}>Facturé à :</h3>
          ...
          <div className="text-right space-y-1">
            <h3 className="border-b-2 border-[#f4f7f8] pb-1 mb-2 font-bold uppercase text-sm" style={{ color: primaryColor }}>Détails :</h3>
            <div className="text-gray-500 text-sm">Date : <span contentEditable suppressContentEditableWarning>30 Mars 2026</span></div>
            <div className="text-gray-500 text-sm">Paiement : <span contentEditable suppressContentEditableWarning>Virement</span></div>
          </div>
        </div>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 group">
                <td className="p-4 outline-none text-sm" contentEditable suppressContentEditableWarning onBlur={(e) => updateItem(item.id, 'desc', e.target.innerText)}>{item.desc}</td>
                <td className="p-4 text-center">
                  <input type="number" className="w-16 text-center bg-transparent outline-none" value={item.qty} onChange={(e) => updateItem(item.id, 'qty', parseInt(e.target.value) || 0)} />
                </td>
                <td className="p-4 text-right">
                  <input type="number" className="w-24 text-right bg-transparent outline-none" value={item.price} onChange={(e) => updateItem(item.id, 'price', parseFloat(e.target.value) || 0)} />
                </td>
        ...
        <div className="p-10 flex justify-end">
          <div className="w-72 space-y-2">
            ...
            <div className="flex justify-between text-gray-600 text-sm items-center">
              <span>TVA (<span contentEditable suppressContentEditableWarning onBlur={(e) => setTvaRate(parseInt(e.target.innerText) || 0)}>{tvaRate}</span>%) :</span>
              <span>{tvaAmount.toLocaleString()} CFA</span>
            </div>
            <div className="flex justify-between text-white p-4 rounded mt-4 font-bold text-lg transition-colors duration-500" style={{ backgroundColor: primaryColor }}>
              <span>TOTAL :</span>
        ...
        <div className="bg-[#f4f7f8] p-6 text-center text-[10px] text-gray-400">
           <p contentEditable suppressContentEditableWarning>Merci de votre confiance ! Votre Entreprise SAS | SIRET: 123 456 789 | www.votre-site.com</p>
        </div>
      </div>
      ...
      <style jsx>{`
        @media print {
          ...
          .invoice-card { 
            box-shadow: none !important; border: none !important; max-width: 100% !important; margin: 0 !important;
