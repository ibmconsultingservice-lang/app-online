...
export default function DocRepairer() {
  ...
  const downloadFile = async (type) => {
    // On nettoie les éventuels résidus de Markdown avant export
    ...
    <main className="min-h-screen bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] py-12 px-6 font-sans antialiased text-slate-900">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          {/* Input Panel - L'Atelier */}
          <section className="bg-white/70 backdrop-blur-md p-8 rounded-[3rem] shadow-2xl shadow-slate-900/5 border border-white flex flex-col gap-8 transition-all hover:shadow-blue-900/10">
            ...
            <button 
              ...
              className={`w-full py-6 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 shadow-xl flex items-center justify-center gap-3
                ${loading ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-blue-600 hover:-translate-y-1 hover:shadow-blue-500/20 active:translate-y-0'}
              `}
            >
              {loading ? (
                <>
          ...
          <section className="bg-white p-2 rounded-[3.5rem] shadow-2xl shadow-blue-900/10 border border-white flex flex-col min-h-[600px] overflow-hidden">
                   ...
                   <div className="flex gap-3 animate-in slide-in-from-right-4 duration-500">
                     {(result.format === 'pdf' || result.format === 'docx' || result.format === 'txt' || !result.format) && (
                       <button 
                        ...
                        className="group flex items-center gap-2 bg-white text-slate-900 text-[10px] font-black px-6 py-3 rounded-full hover:bg-blue-600 hover:text-white transition-all active:scale-95 shadow-lg"
                       >
                         WORD
                         <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 opacity-50 group-hover:translate-y-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                     ...
                     {(result.format === 'xlsx' || result.format === 'xls' || result.format === 'csv') && (
                       <button 
                        ...
                        className="group flex items-center gap-2 bg-green-500 text-white text-[10px] font-black px-6 py-3 rounded-full hover:bg-green-400 transition-all active:scale-95 shadow-lg"
                       >
                         EXCEL
                         <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 opacity-50 group-hover:translate-y-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
             ...
             <div className="flex-1 px-10 py-8 text-sm text-slate-600 overflow-y-auto whitespace-pre-wrap font-medium leading-loose selection:bg-blue-100 selection:text-blue-900">
               {result.content ? (
                 <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                   {result.content}
                 </div>
               ) : (
                 <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-20 group">
                   <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                    <svg className="w-10 h-10 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10l5 5v11a2 2 0 01-2 2z"></path>
                    </svg>
                   </div>
