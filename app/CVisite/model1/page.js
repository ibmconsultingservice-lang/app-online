...
export default function BusinessCardGenerator() {
  const [cardData, setCardData] = useState({
    ...
    logo: null // Stockera l'image en Base64
  })

  const [primaryColor, setPrimaryColor] = useState('#00aeef')
  const iframeRef = useRef(null)

  // Synchronisation avec l'iframe pour les couleurs et logos
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen bg-[#f1f5f9] p-8 flex flex-col md:flex-row gap-8">
      ...
      <div className="w-full md:w-1/3 bg-white p-6 rounded-2xl shadow-xl space-y-4 h-fit">
        <h2 className="text-xl font-bold text-slate-800 border-b pb-2">Personnalisation</h2>
        ...
        <button onClick={() => iframeRef.current.contentWindow.print()} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-blue-600 transition-all mt-6 shadow-lg">
