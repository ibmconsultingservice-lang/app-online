...
import {
  ...
  ChevronRight, ChevronLeft, Edit3, Image, Type, Square,
  Circle, Minus, ArrowRight, Clock, Hash, Target
} from 'lucide-react'
...
// ── Steps ──
const STEPS = ['brief', 'analyse', 'script', 'visuel']
...
export default function PostGenerator() {
  ...
  const handleFinalise = async () => {
    ...
    if (data) {
      ...
      setStep(3)
      // Auto-populate visual with post text
      if (data.post) {
        setElements([{
  ...
  const exportCanvas = () => {
    ...
    import('html2canvas').then(({ default: html2canvas }) => {
      html2canvas(el).then(canvas => {
        ...
        a.download = `post-${form.platform}-${Date.now()}.png`
        a.click()
      })
    })
  }
        ...
        {step === 0 && (
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Ex: Lancement de notre nouveau produit SaaS, conseils pour augmenter sa productivité..."
                  rows={3}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Ex: Mentionner notre offre Black Friday, inclure une statistique clé..."
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-pink-500/50 transition-all"
                />
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading || !form.theme.trim() || !form.audience.trim()}
        ...
        {step === 3 && finalPost && (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
                ...
                {hashtags.length > 0 && (
                  <div className="pt-3 border-t border-white/5 flex flex-wrap gap-1.5">
                    {hashtags.map((h, i) => (
                      <span key={i} className="text-xs text-blue-400 font-bold hover:text-blue-300 cursor-pointer">{h}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              <button
                onClick={() => { setStep(0); setStrategy(null); setScript(null); setFinalPost(null) }}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-500 text-xs mt-0.5">Créez votre visuel — glissez, redimensionnez, éditez</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={exportCanvas} className="flex items-center gap-1.5 px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold transition-all">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] text-slate-600 uppercase tracking-widest mr-1">Ajouter :</span>
                <button onClick={() => addElement('text')} className="flex items-center gap-1 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold transition-all">
                  <Type size={11} /> Texte
                </button>
                <button onClick={() => addElement('rect')} className="flex items-center gap-1 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold transition-all">
                  <Square size={11} /> Rectangle
                </button>
                <button onClick={() => addElement('circle')} className="flex items-center gap-1 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold transition-all">
                  <Circle size={11} /> Cercle
                </button>
                <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold transition-all">
                  <Image size={11} /> Image
                </button>
                <button onClick={() => bgFileRef.current?.click()} className="flex items-center gap-1 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-bold transition-all">
                  <Minus size={11} /> Fond
                </button>
                <input
                ...
                {selected && (
                  <>
                    ...
                    <input type="color" value={selectedEl?.color || '#3b82f6'}
                      ...
                      className="w-7 h-7 rounded cursor-pointer border-0" title="Couleur élément" />
                    <select
                    ...
                    >
                      {[10, 12, 14, 16, 18, 22, 28, 36, 48].map(s => <option key={s} value={s}>{s}px</option>)}
                    </select>
                    <button onClick={deleteEl} className="p-1.5 bg-red-500/20 hover:bg-red-500/40 text-red-400 rounded-lg">
                      <Trash2 size={11} />
                    </button>
                  </>
                )}
              ...
              >
                {elements.map(el => {
                  ...
                  if (el.type === 'image') return (
                    <div key={el.id} style={{ ...baseStyle, outline: isSelected ? '2px solid #ec4899' : 'none', borderRadius: 4 }}
                      ...
                      {isSelected && (
                        <div style={{ position: 'absolute', bottom: -4, right: -4, width: 12, height: 12, background: '#fff', borderRadius: 3, cursor: 'nwse-resize', border: '1px solid #888' }}
                  ...
                  if (el.type === 'circle') return (
                    <div key={el.id}
                      style={{ ...baseStyle, borderRadius: '50%', background: el.color, opacity: el.bgOpacity, display: 'flex', alignItems: 'center', justifyContent: 'center', outline: isSelected ? '2px solid #ec4899' : 'none' }}
                      onMouseDown={e => startElDrag(e, el)}
                      onDoubleClick={e => startElEdit(e, el)}
                      onClick={e => { e.stopPropagation(); setSelected(el.id) }}>
                      {isEditing ? (
                        <input autoFocus value={editingElText} onChange={e => setEditingElText(e.target.value)}
                          onBlur={commitElEdit} style={{ background: 'transparent', color: '#fff', textAlign: 'center', fontSize: el.fontSize, fontWeight: el.bold ? 700 : 400, border: 'none', outline: 'none', width: '90%' }} />
                      ) : (
                        <span style={{ color: '#fff', fontSize: el.fontSize, fontWeight: el.bold ? 700 : 400 }}>{el.text}</span>
                      )}
                      {isSelected && <div style={{ position: 'absolute', bottom: 0, right: 0, width: 12, height: 12, background: '#fff', borderRadius: 3, cursor: 'nwse-resize', border: '1px solid #888' }} onMouseDown={e => startElResize(e, el)} />}
                    </div>
                  )
                  ...
                  return (
                    <div key={el.id}
                      style={{
                        ...baseStyle, borderRadius: el.type === 'rect' ? 8 : 0,
                        background: el.bgOpacity > 0 ? el.color : 'transparent',
                        opacity: 1,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        outline: isSelected ? '2px solid #ec4899' : el.type === 'text' ? '1px dashed rgba(255,255,255,0.15)' : 'none',
                        padding: 8,
                      }}
                      onMouseDown={e => startElDrag(e, el)}
                      onDoubleClick={e => startElEdit(e, el)}
                      ...
                      {isEditing ? (
                        <textarea autoFocus value={editingElText} onChange={e => setEditingElText(e.target.value)}
                          onBlur={commitElEdit}
                          style={{ background: 'transparent', color: '#fff', fontSize: el.fontSize, fontWeight: el.bold ? 700 : 400, border: 'none', outline: 'none', width: '100%', height: '100%', resize: 'none', textAlign: 'center' }} />
                      ) : (
                        <span style={{ color: el.bgOpacity > 0 ? '#fff' : el.color, fontSize: el.fontSize, fontWeight: el.bold ? 700 : 400, textAlign: 'center', lineHeight: 1.4, wordBreak: 'break-word' }}>{el.text}</span>
                      )}
                      {isSelected && <div style={{ position: 'absolute', bottom: -4, right: -4, width: 12, height: 12, background: '#fff', borderRadius: 3, cursor: 'nwse-resize', border: '1px solid #888', zIndex: 10 }} onMouseDown={e => startElResize(e, el)} />}
                    </div>
                  )
                })}
                ...
                {elements.length === 0 && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <p className="text-white/20 text-xs font-bold">Ajoutez des éléments avec la barre d'outils</p>
                  </div>
                )}
              </div>

              <p className="text-[10px] text-slate-700 text-center">Double-clic: éditer • Glisser: déplacer • Coin: redimensionner • Del: supprimer</p>
            </div>
          </div>
        )}
      </div>
