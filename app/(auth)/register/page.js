...
export default function RegisterPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      await loginGoogle()
      // redirect handled by getRedirectResult in useAuth
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] flex items-center justify-center px-4 font-sans overflow-hidden relative">

      {/* Background blobs */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] right-[-5%] w-[40%] h-[40%] bg-indigo-100/50 blur-[120px] rounded-full"></div>
        <div className="absolute bottom-[-10%] left-[-5%] w-[40%] h-[40%] bg-blue-100/50 blur-[120px] rounded-full"></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-xl shadow-slate-200/50 p-10">
          ...
          <div className="flex flex-col items-center mb-10">
            ...
            <p className="text-slate-500 text-xs font-medium mt-2 uppercase tracking-widest">
              Créez votre compte — <span className="text-indigo-600 font-bold">10 crédits offerts 🎁</span>
            </p>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 border border-slate-200 rounded-xl py-3 text-sm font-semibold text-slate-700 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50/50 transition-all mb-6 disabled:opacity-50"
          >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-3 text-[11px] font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-500/25 disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
            >
              {loading ? 'Création...' : <>Créer mon compte gratuitement <ArrowRight size={14}/></>}
            </button>
          </form>
