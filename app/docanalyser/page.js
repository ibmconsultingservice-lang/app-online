...
export default function DocAnalyser() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await fetch('/api/generer-docanalyser', { method: 'POST', body: formData })
  ...
  const handleDeepAnalyse = async () => {
    ...
    try {
      const res = await fetch('/api/generer-docanalyser/deepanalyse', { method: 'POST', body: formData })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      await deductCredits(2)
  ...
  // ── Loading screen ──
  if (!allowed) return (
    <div className="min-h-screen bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] flex items-center justify-center flex-col gap-4">
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <header className="relative bg-white/60 backdrop-blur-xl p-10 rounded-[3rem] shadow-2xl shadow-blue-900/5 border border-white/80 flex flex-col md:flex-row justify-between items-center gap-6 overflow-hidden">
          ...
          <div className="relative z-10 flex items-center gap-3">
            {/* Credits badge */}
            <div className="flex items-center gap-1.5 bg-indigo-50 border border-indigo-100 rounded-full px-4 py-2">
            ...
            {file && (
              <div className="flex items-center gap-3 bg-white px-5 py-3 rounded-2xl shadow-inner border border-slate-100 animate-in fade-in zoom-in duration-300">
        ...
        {credits < 2 && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3 text-xs text-amber-700 font-medium flex items-center justify-between">
            <span>⚠️ Crédits insuffisants (2 requis par analyse)</span>
          ...
          <section className="bg-white/70 backdrop-blur-md p-8 rounded-[3rem] shadow-2xl shadow-slate-900/5 border border-white flex flex-col">
            <div className="space-y-4 mb-8">
              ...
              <input type="file" onChange={(e) => setFile(e.target.files[0])}
                className="w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-8 file:rounded-2xl file:border-0 file:text-xs file:font-black file:uppercase file:tracking-widest file:bg-slate-900 file:text-white hover:file:bg-green-600 cursor-pointer" />
            </div>
            <textarea value={manualText} onChange={(e) => setManualText(e.target.value)}
              placeholder="Collez votre texte ici..."
              className="w-full flex-1 min-h-[320px] p-6 rounded-[2rem] bg-slate-50/50 border-2 border-transparent focus:border-blue-500/20 focus:bg-white transition-all text-sm text-slate-700 resize-none outline-none mb-8" />
            <button onClick={handleProcess} disabled={loading || !hasCredits(2)}
              className={`w-full py-6 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-xl flex items-center justify-center gap-3 ${
                loading || !hasCredits(2) ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-green-600'
              }`}>
              {loading ? "Analyse en cours..." : "Lancer l'analyse · ⚡2"}
            </button>
          </section>
            ...
            <div className="bg-slate-900 m-2 p-8 rounded-[2.5rem] flex justify-between items-center">
              ...
              {result.content && (
                <button onClick={() => downloadFile('word')}
                  className="bg-white text-slate-900 text-[10px] font-black px-6 py-3 rounded-full hover:bg-green-600 hover:text-white transition-all">
                  EXPORTER WORD
                </button>
              )}
            </div>
        ...
        <section className="mt-20 space-y-10">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-stretch">
            <section className="bg-white/70 backdrop-blur-md p-8 rounded-[3rem] shadow-xl border border-white flex flex-col">
              <div className="space-y-4 mb-8">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 italic">Question Spécifique</label>
              ...
              />
              <button onClick={handleDeepAnalyse} disabled={deepLoading || !deepPrompt || !hasCredits(2)}
                className={`w-full py-6 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-lg flex items-center justify-center gap-3 ${
                  deepLoading || !hasCredits(2) ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-green-600'
                }`}>
                {deepLoading ? "Investigation..." : "Lancer l'investigation · ⚡2"}
              </button>
            </section>
              ...
              <div className="bg-slate-900 m-2 p-8 rounded-[2.5rem] flex justify-between items-center">
                ...
                {deepLoading && (
                  <div className="flex gap-1">
                    ...
                    <div className="w-1 h-1 bg-green-500 rounded-full animate-bounce [animation-delay:-.3s]"/>
                    <div className="w-1 h-1 bg-green-500 rounded-full animate-bounce [animation-delay:-.5s]"/>
                  </div>
                )}
              </div>
              <div className="flex-1 px-10 py-8 text-sm text-slate-600 overflow-y-auto whitespace-pre-wrap leading-loose font-medium">
                {deepResult ? (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">{deepResult}</div>
                ) : (
