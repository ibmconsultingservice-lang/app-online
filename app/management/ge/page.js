...
const MARKET_CRITERIA = {
  ...
  competitionLevel: { label: 'Intensité concurrentielle', desc: 'Nb et force des acteurs (score inversé)', weight: 0.15 },
  ...
  cyclicality:      { label: 'Cyclicité du marché',       desc: 'Stabilité de la demande (score inversé)', weight: 0.10 },
}
...
// 3×3 zone map — [row][col] (row 0=top, col 0=left)
const ZONE_MAP = [
...
const BUBBLE_COLORS = ['#818cf8','#f472b6','#fb923c','#34d399','#facc15','#60a5fa','#a78bfa','#2dd4bf']
...
export default function McKinseyGEPage() {
  ...
  const createAnalysis = () => {
    ...
    setAnalyses(updated); setActiveId(a.id); setAiResult(null); setView('setup'); setSetupTab('context'); setActiveSbu(0)
  ...
  const deleteAnalysis = (id) => {
    ...
    if (activeId === id) { const last = updated[updated.length - 1]; setActiveId(last?.id || null); setAiResult(last?.aiResult || null); setView(last?.aiResult ? 'report' : 'setup') }
  ...
  const getSbuScores = (sbu) => {
    ...
    const z = getZoneMeta(a, s)
    return { a, s, zone: ZONE_MAP[z.row][z.col] }
  }
  ...
  const runAI = async () => {
    ...
    try {
      const res  = await fetch('/api/generer-management/generer-ge', {
        ...
        body: JSON.stringify({ projectName: project?.name, projectTag: project?.tag, companyName: active.companyName, context: active.context, sbus: active.sbus.filter(s => s.name.trim()) }),
      })
  ...
  const exportAnalysis = () => {
    ...
    a.href = url; a.download = `McKinseyGE_${active.name.replace(/\s+/g,'_')}.json`; a.click(); URL.revokeObjectURL(url)
  }
  ...
  // ── Matrix SVG computation
  const matrixBubbles = validSbus.map((sbu, i) => {
    const a = computeScore(sbu.marketScores,   MARKET_CRITERIA)
    const s = computeScore(sbu.strengthScores, STRENGTH_CRITERIA)
    // x = strength (left=low, right=high), y = attractiveness (top=high, bottom=low)
    const x = ((s - 1) / 4) * 100
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          ...
          --invest:#34d399; --selective:#facc15; --harvest:#f87171;
        }
        ...
        .sbu-tab:hover { border-color:var(--border2); background:var(--surface3); }
        ...
        .sbu-del { background:none; border:none; color:var(--muted); cursor:pointer; font-size:11px; padding:3px; border-radius:4px; transition:color .15s; }
        ...
        .star { font-size:8px; }
        .sbu-live-scores { display:flex; gap:12px; padding:10px 20px; background:var(--surface2); border-top:1px solid var(--border); }
        ...
        /* ── McKinsey Matrix SVG wrap ── */
        .matrix-svg-wrap { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:20px; }
        ...
        .portfolio-header { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:20px; display:flex; gap:20px; align-items:center; }
        ...
        .portfolio-summary { flex:1; font-size:13px; line-height:1.75; color:var(--muted2); }
        ...
        .sbu-report-hdr { padding:16px 20px; border-bottom:1px solid var(--border); display:flex; align-items:center; justify-content:space-between; }
        .sbu-report-name { font-family:'Instrument Serif',serif; font-size:20px; font-style:italic; }
        .sbu-report-meta { display:flex; gap:10px; align-items:center; }
        ...
        .move-card { background:var(--surface); border:1px solid var(--border); border-radius:8px; padding:14px; display:flex; gap:10px; }
        ...
        .move-tags { display:flex; gap:6px; margin-top:6px; }
        ...
        <div className="body">
          ...
          <aside className="left-panel">
            ...
            <div className="panel-list">
              {analyses.length === 0 && <div className="empty-cta"><div className="empty-icon">🔷</div><div className="empty-txt">Créez votre première analyse McKinsey/GE</div></div>}
              {analyses.map(a => (
                <div key={a.id} className={`litem ${activeId === a.id ? 'active' : ''}`}
                  onClick={() => { setActiveId(a.id); setAiResult(a.aiResult || null); setView(a.aiResult ? 'report' : 'setup'); if (a.aiResult) setActiveSbuR(a.sbus?.[0]?.id) }}>
                  <button className="litem-del" onClick={e => { e.stopPropagation(); deleteAnalysis(a.id) }}>✕</button>
                  <div className="litem-name">{a.name}</div>
                  <div className="litem-meta">{new Date(a.createdAt).toLocaleDateString('fr-FR', { day:'2-digit', month:'short' })} · {(a.sbus || []).filter(s => s.name).length} DAS{a.aiResult ? ' · IA ✓' : ''}</div>
                </div>
              ))}
            </div>
            {showNewForm ? (
              <div className="new-form">
                ...
                <input className="input" placeholder="Ex: Portefeuille 2026" value={newForm.name}
          ...
          <main className="center-panel">
            {!active ? (
              <div className="empty-cta" style={{ padding:'80px 40px', textAlign:'center' }}>
                ...
                <div style={{ fontFamily:'Instrument Serif,serif', fontSize:22, fontStyle:'italic', marginBottom:8 }}>Matrice McKinsey / GE</div>
                <div className="empty-txt">Évaluez votre portefeuille de DAS avec précision</div>
              </div>
            ) : view === 'setup' ? (
              /* ══ SETUP ══ */
              <>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
                  ...
                  <div className="tab-strip">
                    ...
                    <button className={`tab-btn ${setupTab === 'sbus'      ? 'active' : ''}`} onClick={() => setSetupTab('sbus')}>DAS {(active.sbus||[]).length > 0 ? `(${(active.sbus||[]).length})` : ''}</button>
                  ...
                  <div className="card">
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                        ...
                        <input className="input" placeholder="Ex: Industrie manufacturière, SaaS B2B…" value={active.sector || ''} onChange={e => updateActive({ sector: e.target.value })} />
                      </div>
                    </div>
                    <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                      <label className="lbl">Contexte stratégique</label>
                      <textarea className="input" rows={3} placeholder="Enjeux stratégiques actuels, objectifs de portefeuille, contraintes de ressources…" value={active.context || ''} onChange={e => updateActive({ context: e.target.value })} />
                    </div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                    <div className="card">
                      <div className="card-title" style={{ justifyContent:'space-between' }}>
                        <span style={{ display:'flex', alignItems:'center', gap:8 }}><div className="card-title-dot"/>Domaines d'Activité Stratégiques ({(active.sbus||[]).length})</span>
                            ...
                            <div key={sbu.id} className={`sbu-tab ${activeSbu === i ? 'active' : ''}`} onClick={() => { setActiveSbu(i); setSetupTab('scoring') }}>
                              ...
                              <div style={{ flex:1 }}>
                                <input className="input" style={{ background:'transparent', border:'none', padding:0, fontSize:13, fontWeight:600 }}
                                  placeholder={`DAS ${i + 1} — cliquez pour nommer`} value={sbu.name}
                    ...
                    {(active.sbus||[]).map((sbu, i) => (
                      activeSbu === i && (
                        <div key={sbu.id} className="card">
                          <div className="card-title"><div className="card-title-dot" style={{ background: BUBBLE_COLORS[i % BUBBLE_COLORS.length] }}/>Description du DAS</div>
                          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                            <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                              <label className="lbl">Description</label>
                              <textarea className="input" rows={2} placeholder="Activité, produits, marchés couverts…" value={sbu.description || ''} onChange={e => updateSbu(i, { description: e.target.value })} />
                            </div>
                            <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                              <label className="lbl">Notes / observations</label>
                              <textarea className="input" rows={2} placeholder="Contexte spécifique, signaux…" value={sbu.notes || ''} onChange={e => updateSbu(i, { notes: e.target.value })} />
                            </div>
                          </div>
                    ...
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                      {(active.sbus||[]).map((sbu, i) => (
                        <button key={sbu.id} onClick={() => setActiveSbu(i)}
                          style={{ padding:'6px 14px', borderRadius:8, fontSize:11, fontFamily:'Geist Mono,monospace', border:`1px solid ${activeSbu === i ? BUBBLE_COLORS[i % BUBBLE_COLORS.length] + '60' : 'var(--border2)'}`, background: activeSbu === i ? BUBBLE_COLORS[i % BUBBLE_COLORS.length] + '12' : 'var(--surface2)', color: activeSbu === i ? BUBBLE_COLORS[i % BUBBLE_COLORS.length] : 'var(--muted2)', cursor:'pointer', transition:'all .15s' }}>
                          {sbu.name || `DAS ${i + 1}`}
                        </button>
                      ))}
                    </div>
                      ...
                      <div className="score-editor">
                        <div className="score-editor-header">
                          ...
                          <div className="score-type-tabs">
                            <button className={`score-type-tab ${scoreTab === 'market'   ? 'active' : ''}`} onClick={() => setScoreTab('market')}>🌐 Attractivité marché</button>
                            <button className={`score-type-tab ${scoreTab === 'strength' ? 'active' : ''}`} onClick={() => setScoreTab('strength')}>💪 Force compétitive</button>
                          </div>
                        </div>

                        <div className="score-list">
                          {scoreTab === 'market'
                            ? Object.entries(MARKET_CRITERIA).map(([key, meta]) => {
                                const val = currentSbu.marketScores?.[key] ?? 3
                                return (
                                  <div key={key} className="score-row">
                                    ...
                                    <input type="range" className="cr" min={1} max={5} step={0.5} value={val}
                                      ...
                                      onChange={e => setScore(activeSbu, 'market', key, parseFloat(e.target.value))} />
                            ...
                            : Object.entries(STRENGTH_CRITERIA).map(([key, meta]) => {
                                const val = currentSbu.strengthScores?.[key] ?? 3
                                return (
                                  <div key={key} className="score-row">
                                    <div className="score-label-wrap">
                                      <span className="score-crit-label">{meta.label}</span>
                                    ...
                                    <input type="range" className="cr" min={1} max={5} step={0.5} value={val}
                                      style={{ background:`linear-gradient(to right, ${sc(val)} ${((val-1)/4)*100}%, var(--surface3) 0%)` }}
                                      onChange={e => setScore(activeSbu, 'strength', key, parseFloat(e.target.value))} />
                                    <div className="score-val-wrap">
                                      <span className="score-val" style={{ color: sc(val) }}>{val.toFixed(1)}</span>
                                      <div className="score-stars">{[1,2,3,4,5].map(n => <span key={n} className="star" style={{ color: n <= Math.round(val) ? sc(val) : 'var(--surface3)' }}>●</span>)}</div>
                                    </div>
                          ...
                          return (
                            <div className="sbu-live-scores">
                              <div className="live-score-item">
                                <span className="live-score-val" style={{ color: sc(a) }}>{a.toFixed(2)}</span>
                                <span className="live-score-lbl">Attractivité</span>
                              </div>
                              <div style={{ width:1, background:'var(--border)', margin:'0 4px' }}/>
                              <div className="live-score-item">
                                <span className="live-score-val" style={{ color: sc(s) }}>{s.toFixed(2)}</span>
                                <span className="live-score-lbl">Force compét.</span>
                              </div>
                              <div style={{ width:1, background:'var(--border)', margin:'0 4px' }}/>
                              <div style={{ display:'flex', alignItems:'center', gap:8, marginLeft:4 }}>
                                <span className="zone-badge" style={{ background:zc.fill, color:zc.text, border:`1px solid ${zc.stroke}`, padding:'5px 14px', borderRadius:20, fontFamily:'Geist Mono,monospace', fontSize:11, fontWeight:700 }}>
                                  {zc.label}
                                </span>
                              </div>
                ...
                {setupTab === 'preview' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                    ...
                    {/* DAS summary table */}
                    <div className="card">
                      <div className="card-title"><div className="card-title-dot"/>Récapitulatif des DAS</div>
              ...
              aiLoading ? (
                <div className="loading-wrap">
                  ...
                  <div style={{ fontSize:13, color:'var(--muted2)', fontFamily:'Geist Mono,monospace' }}>Analyse stratégique McKinsey/GE en cours…</div>
                ...
                <>
                          ...
                          <div className="sbu-report-card" style={{ borderColor: zc.stroke }}>
                            <div className="sbu-report-hdr">
                              ...
                              <div className="sbu-report-meta">
                                ...
                                {activeSbuResult.investment_level && (
                                  <span style={{ fontSize:11, fontFamily:'Geist Mono,monospace', color:'var(--muted2)', padding:'5px 12px', borderRadius:8, border:'1px solid var(--border2)', background:'var(--surface2)' }}>
                                    Invest : {activeSbuResult.investment_level}
                                  </span>
                                )}
                                {activeSbuResult.risk && (
                                  <span style={{ fontSize:11, fontFamily:'Geist Mono,monospace', color: sevC(activeSbuResult.risk), padding:'4px 10px', borderRadius:8, border:`1px solid ${sevC(activeSbuResult.risk)}30`, background:`${sevC(activeSbuResult.risk)}10` }}>
                                    ⚠ {activeSbuResult.risk}
                                  </span>
                                )}
                              </div>
                  ...
                  {reportTab === 'alloc' && aiResult.resource_allocation?.length > 0 && (
                    <div>
                      ...
                      <div className="alloc-list">
                        {[...aiResult.resource_allocation].sort((a, b) => a.budget_priority - b.budget_priority).map((a, i) => (
                  ...
                  {reportTab === 'moves' && aiResult.portfolio_moves?.length > 0 && (
                    <div>
                      <div className="sec-title" style={{ marginBottom:12 }}>Mouvements stratégiques recommandés <div className="sec-line"/></div>
...
function MatrixSVG({ bubbles = [] }) {
  ...
  // 9 zone cells
  const zones = [
    // row 0 (top = high attract), from left to right
    ...
    <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:20 }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width:'100%', maxWidth:W, display:'block', margin:'0 auto', overflow:'visible' }}>
          ...
          return (
            <g key={`${row}-${col}`}>
              <rect x={x} y={y} width={cellW} height={cellH}
                fill={zc.fill} stroke={zc.stroke} strokeWidth={0.5} />
            </g>
          )
        })}
        ...
        {[
          ...
          return (
            <text key={text}
              x={PAD_L + (col + 0.5) * cellW} y={PAD_T + (row + 0.5) * cellH + 4}
              textAnchor="middle" fill={zc.text} fontSize={10} fontFamily="'Geist Mono',monospace"
        ...
        {[1, 2].map(i => (
          <g key={`grid-${i}`}>
            <line x1={PAD_L + i * cellW} y1={PAD_T} x2={PAD_L + i * cellW} y2={PAD_T + GH} stroke="rgba(255,255,255,.12)" strokeWidth={1} />
            <line x1={PAD_L} y1={PAD_T + i * cellH} x2={PAD_L + GW} y2={PAD_T + i * cellH} stroke="rgba(255,255,255,.12)" strokeWidth={1} />
          </g>
        ))}

        {/* Axis ticks & labels */}
        {ticks.map(t => {
          // X axis (strength: 1=left, 5=right)
          const xPos = PAD_L + ((t - 1) / 4) * GW
          // Y axis (attract: 1=bottom, 5=top)
        ...
        {bubbles.map((b, i) => {
          ...
          return (
            <g key={b.id}>
              <circle cx={cx} cy={cy} r={18} fill={b.color} opacity={0.22} />
