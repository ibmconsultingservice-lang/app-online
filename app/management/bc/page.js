...
export default function BSCPage() {
  ...
  const [viewMode, setViewMode] = useState('tableau') // 'tableau' | 'carte'
  ...
  const createScorecard = () => {
    ...
    const s = {
      ...
      vision: newCard.vision.trim(),
      strategie: newCard.strategie.trim(),
    ...
    showToast(`Scorecard "${s.name}" créée`)
  }
  ...
  const computeScore = (objectives) => {
    ...
    const weightedScore = scored.reduce((s, o) => {
  ...
  const perspectiveScore = (key) => {
    ...
    const objs = (active.objectives || []).filter(o => o.perspective === key)
    return computeScore(objs)
  }
  ...
  const exportCard = () => {
    ...
    a.href = url; a.download = `BSC_${active.name.replace(/\s+/g, '_')}.json`
    a.click(); URL.revokeObjectURL(url)
  }

  const getProgressWidth = (current, target, unite) => {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Perspective tabs ── */
        .perspective-tabs { display: flex; gap: 4px; flex-wrap: wrap; }
        .p-tab {
          display: flex; align-items: center; gap: 6px; padding: 6px 12px;
          border-radius: 6px; cursor: pointer; border: 1px solid var(--border);
          background: var(--surface2); font-size: 11px; font-family: 'Geist Mono', monospace;
          color: var(--muted2); transition: all .15s;
        }
        .p-tab:hover { border-color: var(--border2); color: var(--text); }
        .p-tab.active { color: var(--text); border-color: var(--border2); background: var(--surface3); }
        .p-tab-dot { width: 7px; height: 7px; border-radius: 50%; }
        .p-tab-score { font-size: 9px; padding: 1px 5px; border-radius: 3px; font-weight: 700; margin-left: 2px; }
        ...
        .obj-table td {
          padding: 10px 12px; font-size: 12px; border-bottom: 1px solid var(--border);
        ...
        .p-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; margin-right: 6px; }
        .icon-btn {
        ...
        .obj-card-perspective { font-size: 10px; padding: 2px 7px; border-radius: 4px; font-family: 'Geist Mono', monospace; font-weight: 600; }
        ...
        .p-select-item.selected { }
        .p-select-label { font-size: 11px; font-weight: 600; }
          ...
          <aside className="scorecards-panel">
            ...
            <div className="panel-list">
              {scorecards.length === 0 && (
                <div className="empty-cta">
                  ...
                  <div className="empty-txt">Créez votre première scorecard ci-dessous</div>
                </div>
              )}
              {scorecards.map(s => {
                const score = computeScore(s.objectives)
                const scoreColor = score === null ? 'var(--muted)' : score >= 70 ? '#22d3a5' : score >= 40 ? '#f59e0b' : '#f87171'
                return (
                  <div
                    key={s.id}
            ...
            {showNewForm ? (
              <div className="new-form">
                <span className="form-label">Nom de la scorecard</span>
                <input
                  className="input"
                  placeholder="Ex: BSC 2025 — Croissance"
                  value={newCard.name}
                  onChange={e => setNewCard(p => ({ ...p, name: e.target.value }))}
                  onKeyDown={e => e.key === 'Enter' && createScorecard()}
                  autoFocus
                />
                <span className="form-label">Vision (optionnel)</span>
                <textarea
                  className="input"
                  placeholder="Vision stratégique à long terme…"
                  value={newCard.vision}
                  onChange={e => setNewCard(p => ({ ...p, vision: e.target.value }))}
                  rows={2}
                />
                <span className="form-label">Stratégie (optionnel)</span>
                <textarea
                  className="input"
                  placeholder="Axes stratégiques prioritaires…"
            ...
            {!active ? (
              <div className="empty-cta" style={{ padding: '80px 40px', textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'Instrument Serif,serif', fontSize: 20, fontStyle: 'italic', marginBottom: 8 }}>
                  Sélectionnez ou créez une scorecard
                </div>
                      ...
                      <div className="score-ring-wrap">
                        ...
                        >
                          <span
                            ...
                            style={{ color: globalScore >= 70 ? '#22d3a5' : globalScore >= 40 ? '#f59e0b' : '#f87171' }}
                          >
                            {globalScore}%
                          </span>
                        </div>
                    ...
                    {Object.entries(PERSPECTIVES).map(([key, p]) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            borderColor: activePerspective === key
                              ? `color-mix(in srgb, ${p.color} 35%, transparent)`
                              : undefined,
                        ...
                        >
                          ...
                          <div className="p-card-bar">
                            <div
                              className="p-card-bar-fill"
                              style={{
                                width: pScore !== null ? `${pScore}%` : '0%',
                                background: p.color,
                              }}
                            />
                          </div>
                  ...
                  {filteredObjectives.length === 0 ? (
                    <div className="empty-cta">
                      ...
                      <div className="empty-txt">
                        Aucun objectif{activePerspective !== 'all' ? ` dans la perspective "${PERSPECTIVES[activePerspective]?.label}"` : ''}.<br />
                        Ajoutez-en un via le bouton "Ajouter".
                      </div>
                    </div>
                  ) : viewMode === 'tableau' ? (
                          ...
                          {filteredObjectives.map(o => {
                            ...
                            return (
                              <tr key={o.id}>
                                ...
                                <td>
                                  <span className="obj-kpi">{o.kpi || '—'}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                  <span style={{ fontSize: 11, color: 'var(--muted2)' }}>{o.responsable || '—'}</span>
                                </td>
                                <td>
                        ...
                        return (
                          <div key={o.id} className="obj-card" style={{ borderColor: `color-mix(in srgb, ${persp?.color} 15%, transparent)` }}>
                            <div className="obj-card-header">
                              ...
                              <span
                                className="obj-card-perspective"
                                style={{ background: persp?.bg, color: persp?.color }}
                              >
                                {persp?.icon}
                              </span>
                            </div>
              ...
              <div className="form-scroll">
                  ...
                  <div className="perspective-select-grid">
                    {Object.entries(PERSPECTIVES).map(([key, p]) => (
                      ...
                      >
                        ...
                        <span className="p-select-label" style={{ color: objForm.perspective === key ? p.color : 'var(--muted2)', fontSize: 10 }}>{p.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="form-group">
                  <label className="form-label">
                    Poids dans le score — <span className="poids-display">{objForm.poids}x</span>
                  </label>
                  <input
                    type="range" className="range-input" min="1" max="5" step="1"
                    value={objForm.poids}
          ...
          <div className="ai-content">
            ...
            {!aiLoading && !aiResult && (
              <div className="empty-cta">
                <div className="empty-icon">✦</div>
                <div className="empty-txt">
                  Cliquez sur "Analyse IA" pour obtenir une évaluation stratégique de votre Balanced Scorecard.
                </div>
              </div>
            )}
          </div>
